    ...
    render(){
        return(
            <Menu fluid inverted stackable borderless attached size='huge' style={{marginBottom:'5px', backgroundColor:'#3D4853'}}>
                ...
                <Popup inverted position='right center' trigger={<Label color='red' style={{textDecoration:'underline'}}>Alpha</Label>} content='Alpha 0.2.15 (26-Dec-2018)'/></Menu.Item>
                {this.props.cdps ? <Menu.Item fluid inverted><Search fluid selectFirstResult value={this.state.value} loading={this.state.isLoading} onSearchChange={this.handleSearchChange} onResultSelect={this.props.handleSearchClick} results = {this.state.results} placeholder={"Search by CDP ID"} size='mini'/></Menu.Item> : null}
