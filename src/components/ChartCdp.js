	...
	return ({ currentItem, xAccessor }) => {
		return {
			...
			y: [
                {
					label: "CDPs Created",
					value: currentItem.cdpsOpened && numberFormat(currentItem.cdpsOpened),
					stroke: '#1678C2'
				},
				{
					label: "Total Activity",
					value: currentItem.actionCount && numberFormat(currentItem.actionCount),
                    stroke: '#6BA583',
...
class AreaChartWithEdge extends React.Component {
	...
	render() {
		...
		return (
			...
			<Chart id={1} yExtents={d => [d.cdpsOpened, 0]}>
				...
				<AreaSeries yAccessor={d => d.cdpsOpened} fill='#1678C2' stroke='#1678C2'/>
				<SingleValueTooltip
						yLabel="Total Activity (Wipe, Lock, Draw, etc.)"
						yAccessor={d => d.actionCount}
						yDisplayFormat={format(".0f")}
						origin={[0, 20]}
						valueFill= "#FFFFFF"
					/>
				<SingleValueTooltip
				...
				<EdgeIndicator itemType='last' orient='left' edgeAt='right' fill={d => d.cdpsOpened > d.open ? "#6BA583" : "#6BA583"} yAccessor={d=> d.cdpsOpened}/>

			</Chart>
			<Chart id={2}
					yExtents={d => d.actionCount}
					height={100}
					origin={(w, h) => [0, h - 100]}>
					...
					<BarSeries yAccessor={d => d.actionCount}
						stroke fill="#6BA583"
						opacity={0.4}
						widthRatio={0.9} />
